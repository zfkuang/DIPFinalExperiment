...
def encoder(x, output_dim, reuse=False):
...
def prototypicalNetwork(sess, trainData, trainLabel, trainIndex, testData, testLabel, sourceClassNumber=1000, novelClassNumber=50, **kwargs):
    ...
    emb_x = encoder(tf.reshape(x, [num_classes * num_support, n_features]), output_dim)
    emb_dim = tf.shape(emb_x)[-1]
    emb_x = tf.reduce_mean(tf.reshape(emb_x, [num_classes, num_support, emb_dim]), axis=1)
    emb_q = encoder(q, output_dim, reuse=True)
    ...
    params = tf.trainable_variables()[-4:]
    print(params)
    ...
    for ep in range(n_epochs):
        for epi in range(n_episodes):
            ...
            _, ls, ac, logy = sess.run([train_op, ce_loss, acc, log_p_y], feed_dict={x: support, q: query, y:labels})
            if (epi+1) % 50 == 0:
                print(logy[:5])
        ...
        for epi in range(n_test_episodes):
            ...
            ac, ls, logit, logy = sess.run([acc, ce_loss, logits, log_p_y], feed_dict={x: support, q: query, y:labels})
            if epi == 0:
                print(logy[:5])
                print(logit[:5])
            acc_.append(ac)
